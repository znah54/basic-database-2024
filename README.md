...
- 실행화면
  ...
  https://github.com/znah54/basic-database-2024/assets/50408406/9991bf9b-4375-4c3d-a385-b0ed49865d48
...
-데이터모델링
    ...
    - ER 모델링 - ERD를 그르기위하 기본 이론 

- 인덱스 예제
    - PK나 인덱스가 없는 상태에서 성능문제 체크
    - 인덱스가 설정되면 성능이 어떤지 비교
    - 더미 생성시 100만건으로 제약 두고 시작
...
- 정규화
    ...
    - 기본키와 함수종속성을 파악
    - 1정규형 - 도메인이 원자값을 가짐
    - 2정규형 - 기본키가 아닌 속성이 기본키에 완전 종속일떄(학생번호[PK], 강좌이름 --> 성적을 결정)
    - 3정규형 - 기본키가 아닌 속성이 기본키에 비이행적으로 종속할때(학생번호 -> 강좌이름 -> 수강료 [이행종속성])
    - BCNF정규형 - 함수 족속성 X -> Y가 성립할때 모든 결정자X가 후보키(기본키가 될 수 있는 속성)이면
    - 4정규형(다치 종속성), 5정규형()
...
- 실무실습(사용자, 권한...)
    1. DB관리자
        ...
        - hrDB를 사용할 사용자 계정을 생성, 필요한 권한 설정
            - 추가 - sa의 비번을 잊어버리면 로그인할때, Windows 인증으로 로그인 후 
...
- 실무실습
    ...
    ![HR_ERD](https://github.com/znah54/basic-database-2024/blob/main/images/db007.png?raw=true)
